...
public class ResponseValidatorTest {
    ...
    private final String protocol = SAMLConstants.SAML20P_NS;
    private static final String DEFAULT_ELEMENT_LOCAL_NAME = "Response";
    private static final QName DEFAULT_ELEMENT_NAME = new QName(SAMLConstants.SAML20P_NS, DEFAULT_ELEMENT_LOCAL_NAME, SAMLConstants.SAML20P_PREFIX);
        ...
        mockery.checking(new Expectations() {
            {
                ...
                one(statusMessage).getValue(); //v5 update
    ...
    public void testVerifyResponseSignature_ThrowsSecurityPolicyException() {
        ...
        try {
             ...
             * [org.opensaml.messaging.handler.MessageHandlerException]. Cause:[The server is configured with the signature method http://www.w3.org/2001/04/xmldsig-more#rsa-sha512
             * but the received SAML assertion is signed with the signature method http://www.w3.org/2000/09/xmldsig#rsa-sha1, the signature method provided is weaker than the
             * required.], StackTrace: [
             */
            //assertEquals("Expected to receive the message for '" + SAML20_AUTHENTICATION_FAIL + "' but it was not received.",
            //             SAML20_AUTHENTICATION_FAIL, ex.getMsgKey());
    ...
    private class MockSignatureTrustEngine implements SignatureTrustEngine {
        ...
        public boolean validate(byte[] signature, byte[] content, String algorithmURI,
                                CriteriaSet trustBasisCriteria, Credential candidateCredential) throws org.opensaml.security.SecurityException {
            if (trusted == null) {
                throw new org.opensaml.security.SecurityException("This means an error happened");
            }
            return trusted;
