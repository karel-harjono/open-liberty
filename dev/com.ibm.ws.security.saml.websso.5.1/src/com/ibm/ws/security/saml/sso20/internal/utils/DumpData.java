...
public class DumpData {
    ...
    public static StringBuffer dumpAssertion(StringBuffer sbParent, Assertion assertion, int identLevel) {
        ...
        sb.append(identString(identLevel, true)).append("SubjectID(Username):").append(assertion.getSubject().getNameID().getValue()).append("\n");
        sb.append(identString(identLevel)).append("Issuer:").append(assertion.getIssuer().getValue()).append("\n");
        sb.append(identString(identLevel)).append("isSigned:").append(assertion.isSigned()).append(" signReferenceId:").append(assertion.getSignatureReferenceID()).append("\n");
        sb.append(identString(identLevel)).append("DOM:").append(assertion.getDOM()).append(" @").append(assertion.getDOM().hashCode()).append(")\n");
    ...
    public static StringBuffer dumpXMLObject(StringBuffer sbParent, XMLObject xmlObject, int identLevel) {
        StringBuffer sb = sbParent == null ? new StringBuffer("\n") : sbParent;
        if (xmlObject == null) {
            sb.append(identString(identLevel, false)).append("found an null XMLObject\n");
            return sb;
        }
        QName qName = xmlObject.getElementQName();
        sb.append(identString(identLevel, true)).append(qName.getPrefix()).append(":").append(qName.getLocalPart()).append("(").append(qName.getNamespaceURI()).append(")\n");
        Element dom = xmlObject.getDOM();
        if (dom != null) {
            sb.append(identString(identLevel)).append("DOM:").append(dom).append(" @").append(dom.hashCode()).append(")\n");
        } else {
            sb.append(identString(identLevel)).append("DOM is null");
        }

        if (xmlObject instanceof SignableSAMLObject) {
            sb.append(identString(identLevel)).append("isSigned:").append(((SignableSAMLObject) xmlObject).isSigned()).append(" id:").append(((SignableSAMLObject) xmlObject).getSignatureReferenceID()).append(")\n");
        }
