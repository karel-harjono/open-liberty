...
public class SAMLRequestTAI implements TrustAssociationInterceptor, UnprotectedResourceService {
    ...
    public TAIResult negotiateValidateandEstablishTrust(javax.servlet.http.HttpServletRequest req,
                                                        javax.servlet.http.HttpServletResponse resp) throws WebTrustAssociationFailedException {
     ...
     */
    @Override
    public boolean isTargetInterceptor(javax.servlet.http.HttpServletRequest req) throws WebTrustAssociationException {
        if (tc.isDebugEnabled()) {
     ...
     */
    boolean findSsoSpSpecificFirst(javax.servlet.http.HttpServletRequest req,
     ...
     */
    @Override
    public boolean isAuthenticationRequired(javax.servlet.http.HttpServletRequest request) {
        if (tc.isDebugEnabled()) {
     ...
     */
    @Override
    public boolean logout(javax.servlet.http.HttpServletRequest request, javax.servlet.http.HttpServletResponse response, String userName) {
        ...
        synchronized (reqSsoSamlServiceRef) {
            ...
            if (samlRequest != null) {
                ...
                if (isSamlSingleLogoutInProgress(request) || servletLogoutPerformsSLO(samlRequest)) { // check whether the new configuration attribute spLogout is set or the IdP initiated logout is taking place)
                    return handleSpCookie((IExtendedRequest) request, response, samlRequest, userName, new SpCookieRetriver(authCacheServiceRef.getService(), request, samlRequest),
                                          bSetSubject);
                }
                ...
                if (findSsoSpSpecificFirst(request, reqSsoSamlServiceRef, Constants.EndpointType.LOGOUT, false)) {
                    ...
                    if (samlRequest != null) {
                        ...
                        if (servletLogoutPerformsSLO(samlRequest)) {
                            return handleSpCookie((IExtendedRequest) request, response, samlRequest, userName,
                                                  new SpCookieRetriver(authCacheServiceRef.getService(), request, samlRequest), bSetSubject);
                        }
                    }
                }
            }
            ...
            while (services.hasNext()) {
                ...
                if (servletLogoutPerformsSLO(samlRequest)) {
                    ...
                    if (delegatedLogout) {
                        ...
                        if (handleSpCookie((IExtendedRequest) request, response, samlRequest, userName,
                                           new SpCookieRetriver(authCacheServiceRef.getService(), request, samlRequest), bSetSubject)) {
                            bSetSubject = true;
                        }
                        break;
            ...
            if (!delegatedLogout) { // existing function
                ...
                while (services.hasNext()) {
                    ...
                    samlRequest.setLocationAdminRef(locationAdminRef); // locationAdminRef is needed for get SpCookieName
                    if (handleSpCookie((IExtendedRequest) request, response, samlRequest, userName, new SpCookieRetriver(authCacheServiceRef.getService(), request, samlRequest),
                                       bSetSubject)) {
                        bSetSubject = true;
                    }
                }
    ...
    private boolean isDelegatedLogoutInProgress(HttpServletRequest request, HttpServletResponse response, SsoRequest samlRequest) {
        ...
        SpCookieRetriver spCookieRetriever = new SpCookieRetriver(authCacheServiceRef.getService(), request, samlRequest);
    ...
    class SsoSamlServiceConfig {
        ...
        public SsoSamlServiceConfig(javax.servlet.http.HttpServletRequest req, ConcurrentServiceReferenceMap<String, SsoSamlService> ssoSamlServiceRef,
                                    boolean bSamlInboundAsWell) {
